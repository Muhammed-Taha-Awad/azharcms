<div class="row g-3">
    ...
    <div class="col-md-6">
        ...
        <input class="form-control" name="item_cta_label" value="{{ data_get($attributes, 'item_cta_label') }}" placeholder="{{ __('Read more') }}" />
    </div>
</div>
    ...
    <fieldset class="border rounded-3 p-3 mb-3">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input class="form-control" name="item_title_{{ $i }}" value="{{ data_get($attributes, 'item_title_' . $i) }}" />
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ __('Tag / Category') }}</label>
                <input class="form-control" name="item_tag_{{ $i }}" value="{{ data_get($attributes, 'item_tag_' . $i) }}" />
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ __('Date / Meta') }}</label>
                <input class="form-control" name="item_date_{{ $i }}" value="{{ data_get($attributes, 'item_date_' . $i) }}" placeholder="{{ __('e.g. 12 Sept 2025') }}" />
            </div>
            <div class="col-md-6">
                <label class="form-label">{{ __('Link URL') }}</label>
                <input class="form-control" name="item_url_{{ $i }}" value="{{ data_get($attributes, 'item_url_' . $i) }}" />
            </div>
            <div class="col-md-12">
                <label class="form-label">{{ __('Excerpt') }}</label>
                <textarea class="form-control" name="item_excerpt_{{ $i }}" rows="2">{{ data_get($attributes, 'item_excerpt_' . $i) }}</textarea>
            </div>
            <div class="col-md-12">
                <label class="form-label">{{ __('Image') }}</label>
                {!! Form::mediaImage('item_image_' . $i, data_get($attributes, 'item_image_' . $i)) !!}
            </div>
        </div>
    </fieldset>
@endfor
