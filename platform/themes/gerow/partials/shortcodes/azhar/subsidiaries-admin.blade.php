...
@for ($i = 1; $i <= 3; $i++)
    <div class="row g-3 mb-2">
        <div class="col-md-6">
            ...
            <input class="form-control" name="header_link_label_{{ $i }}" value="{{ data_get($attributes, 'header_link_label_' . $i) }}" />
        </div>
        <div class="col-md-6">
            <label class="form-label">{{ __('Header link :number URL', ['number' => $i]) }}</label>
            <input class="form-control" name="header_link_url_{{ $i }}" value="{{ data_get($attributes, 'header_link_url_' . $i) }}" />
        </div>
    </div>
@endfor
    ...
    <fieldset class="border rounded-3 p-3 mb-3">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input class="form-control" name="item_category_{{ $i }}" value="{{ data_get($attributes, 'item_category_' . $i) }}" />
            </div>
            <div class="col-md-6">
